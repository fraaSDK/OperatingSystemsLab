...
{ 
	...
	while(1) 
	{
		...
		if (rc != 0) PrintERROR_andExit(rc, "pthread_mutex_lock failed during Deposit.");
		...
		if (rc != 0) PrintERROR_andExit(rc, "pthread_mutex_unlock failed during Deposit.");
...
{ 
	...
	while(1) 
	{
		...
		/* AGGIUNGERE CODICE PER ENTRARE IN SEZIONE CRITICA */
		rc = pthread_mutex_lock(&mutex[indicebanca]);
		if (rc) PrintERROR_andExit(rc, "pthread_mutex_lock failed during Withdraw.");
		...
		/* AGGIUNGERE CODICE PER USCIRE DALLA SEZIONE CRITICA */
		rc = pthread_mutex_unlock(&mutex[indicebanca]);
		if (rc) PrintERROR_andExit(rc, "pthread_mutex_unlock failed during Withdraw.");
...
{ 
	...
	while(1) 
	{
		...
		for (i = 0; i < NUMBANCHE; i++) {
			...
			if (rc) PrintERROR_andExit(rc, "pthread_mutex_lock failed during Banca d'Italia");
		}
		...
		*/
		...
		for (i = NUMBANCHE - 1; i >= 0; i--) {
			...
			if (rc) PrintERROR_andExit(rc, "pthread_mutex_unlock failed during Banca d'Italia");
		}
...
{ 
	...
	/* BancadItalia((void)NULL); */ /* Uguale a sotto, ma invece di creare un nuovo thread, eseguo la funzione nel thread main. */
