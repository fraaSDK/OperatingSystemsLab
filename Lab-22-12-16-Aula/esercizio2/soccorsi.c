...
{ 
	while(1) {
		...
		if (NumSoccorritoriArrivati != 2) {
			/* Non abbastanza soccorritori. */
			pthread_cond_wait(&condInizioSoccorso, &mutex);
		} else {
			/* Ci sono due soccorritori. */
			pthread_cond_signal(&condInizioSoccorso);
			NumSoccorritoriArrivati--;
		}
		...
		if (NumSoccorritoriArrivati == 0) {
			/* Fine soccorso, si risvegliano i thread (anche quello del professore). */
			pthread_cond_broadcast(&condFineSoccorso);
		} else {
			pthread_cond_wait(&condFineSoccorso, &mutex);
		}
